...
<head>
    ...
    <meta name="description" content="Selling Nintendo Switch games">
...
<body>
    ...
    <div class="container col-offset-1">
        ...
        //then show all switch games
        if(isset($_POST['removeFilter']) or empty($_POST['genre'])){
            $sql = "SELECT * FROM games WHERE consoleName='switch'";
        //Else, this means that the form has been used so filter the switch games by genre as well    
        }else {  
            //Set value from form to genre so it can be used in the sql query
            $genre = $_POST['genre'];
            $sql = "SELECT * FROM games WHERE consoleName='switch' AND genre='$genre'";     
        }
